...
for j=0:numRuns-1
    ...
    scatterplot(maxRMSEPred, maxRMSETargets, maxRMSETO, maxRMSETT, sprintf(' Predicted vs. Actual with Largest RMSE(%d) %d runs %d Train', MaxRMSE, numRuns, trainPct  ), 'Predicted sin(x)', 'sin(x)');
    %Min RMSE Scatterplot
    scatterplot(minRMSEPred, minRMSETargets, minRMSETO, minRMSETT, sprintf('Predicted Vs Actual with smallest RMSE(%d) %d runs  %d Train,', MinRMSE, numRuns, trainPct ), 'Predicted sin(x)','sin(x)');
